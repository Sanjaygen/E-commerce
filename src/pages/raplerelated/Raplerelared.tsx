...
import { RAPLEQUERY } from '../raple/query';

const Raplerelated = () => {
    const { data } = useQuery(RAPLEQUERY);
    const { raples } = data || {};
    const { cards} = raples?.data?.[0]?.attributes || {};
                ...
                <Box>
                    <CarouselBox>
                        ...
                        <Card sx={{width:{xs:"232px",md:"300px"},height:{xs:"310px",md:"361px"},background:"#F1F1F1",boxShadow:"none"}}>
                            ...
                            <StyleTypo style={{marginLeft:"15px"}}>{cards?.[1]?.text}</StyleTypo>
