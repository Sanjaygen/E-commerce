...
import { BoxStyled, CardButton, CardButton1, CardDiv, CardTypo, CardTypo1, CardTypo2, CardTypo3, CarouselBox, StyleTypo, StyleTypo1, StyleTypo2, TypoProducts, TypographyAmt, TypographyAmt1 } from './Bagsrelated.styled'
import ShoppingCartOutlinedIcon from '@mui/icons-material/ShoppingCartOutlined';
import { useQuery } from '@apollo/client';
import { BAGSQUERY } from '../bags/query';

const Bagsrelated = () => {
    const { data } = useQuery(BAGSQUERY);
    const { bags } = data || {};
    const { cards} = bags?.data?.[0]?.attributes || {};
    ...
    const Carousel = () => {
        return (
            <>
            <BoxStyled>
                <TypoProducts variant="h4">{bags?.data?.[0]?.attributes?.heading}</TypoProducts>
                    ...
                    <CarouselBox>
                        ...
                        <Card sx={{width:"300px",height:"361px",background:"#F1F1F1",boxShadow:"none",display:{xs:"none",md:"block"}}}>
                            ...
                            <StyleTypo2>{cards?.[3]?.text}</StyleTypo2>
