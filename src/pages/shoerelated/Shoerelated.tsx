...
import { BoxStyled, CardButton, CardButton1, CardDiv, CardTypo, CardTypo1, CardTypo2, CardTypo3, CarouselBox, StyleTypo, StyleTypo1, TypoProducts, TypographyAmt, TypographyAmt1 } from './Shoerelated.styled'
import ShoppingCartOutlinedIcon from '@mui/icons-material/ShoppingCartOutlined';
import { useQuery } from '@apollo/client';
import { SHOEQUERY } from '../shoes/query';

const Shoerelated = () => {
    const { data } = useQuery(SHOEQUERY);
    const { shoes } = data || {};
    const { cards} = shoes?.data?.[0]?.attributes || {};
    ...
    const Carousel = () => {
        return (
            <>
            <BoxStyled>
                <TypoProducts variant="h4">{shoes?.data?.[0]?.attributes?.heading}</TypoProducts>
