...
import { BoxStyled, CardButton, CardButton1, CardDiv, CardTypo, CardTypo1, CardTypo2, CardTypo3, CarouselBox, StyleTypo, StyleTypo1, TypoProducts, TypographyAmt, TypographyAmt1 } from './Scrafrelated.styled'
import ShoppingCartOutlinedIcon from '@mui/icons-material/ShoppingCartOutlined';
import { useQuery } from '@apollo/client';
import { SCRAFQUERY } from '../scraf/query';

const Scrafrelated = () => {
    const { data } = useQuery(SCRAFQUERY);
    const { scrafs } = data || {};
    const { cards} = scrafs?.data?.[0]?.attributes || {};
    ...
    const Carousel = () => {
        return (
            <>
            <BoxStyled>
                <TypoProducts variant="h4">{scrafs?.data?.[0]?.attributes?.heading}</TypoProducts>
